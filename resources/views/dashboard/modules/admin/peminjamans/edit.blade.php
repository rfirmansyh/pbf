...
@section('header', 'Ubah Pengembalian Buku')
@section('breadcrumb')
    <div class="breadcrumb-item active"><a href="#">Peminjaman</a></div>
    <div class="breadcrumb-item">Ubah Peminjaman Buku</div>
@endsection
@section('content-header')
  <div class="row gutters-xs align-items-center">
        <div class="col-md"><h2 class="section-title">Form Ubah Peminjaman Buku</h2></div>
...
@section('content')
    ...
    <div class="row justify-content-center">
        <div class="col-lg-6">
            <form action="{{ route('dashboard.admin.peminjamans.update', $peminjaman) }}" method="POST">
                @csrf @method('PUT')
                <div class="card">
                    <div class="card-body">
                            ...
                            <select 
                                ...
                                data-style="form-control" data-width="100%" data-live-search="true" data-live-search-placeholder="Cari Member...">
                                    @foreach ($members as $member)
                                        <option {{ $peminjaman->member->id  === $member->id ? 'selected' : '' }} value="{{ $member->id }}">{{ $member->name }}</option>
                                    @endforeach
                            </select>
                            @error('member_id')
                                <span class="invalid-feedback" role="alert">
                        ...
                        <div class="form-group">
                            ...
                            <input
                                value="{{ old('borrowed_at') ? old('borrowed_at') : \Carbon\Carbon::parse($peminjaman->borrowed_at)->format('m/d/Y h:i A') }}"
                                name="borrowed_at"
                                type="text"
